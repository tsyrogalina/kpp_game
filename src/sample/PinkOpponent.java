...
public class PinkOpponent implements Opponent {
    ...
    public void init(Map map, double scale, double height, double wight) {
        ...
        this.scale = scale;
        this.map = map;
        goalTile = new Rect(0.5,-scale+0.5,scale-1,scale-1);

        srcR = new Rect(122.5, 102.5, 15, 15);
    ...
    public void update(Rect pacmanR,DIR pacmanDIR, int score) {
        ...
        if(Collision.AABB(destR,pacmanR))
        {
            if(opponentMode == OpponentMode.SCARE) {
                ...
                switch(pacmanDIR)
                {
                    case LEFT:goalTile.x-=(scale*4);break;
                    case RIGHT:goalTile.x+=(scale*4);break;
                    case UP:goalTile.y-=(scale*4);break;
                    case DOWN:goalTile.y+=(scale*4);break;
                }
